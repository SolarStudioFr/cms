...
- **Doutes explicitement signalés par l'utilisateur dans `../REFORGE_PLUGINS_AND_I18N.md`** ("Si tu as le moindre doute pose moi des questions") — points à clarifier au démarrage de l'étape concernée plutôt que tranchés unilatéralement ici :
    ...
    - **58** : ~~c'est le point le plus explicitement ouvert du document source...~~ — tranché avec l'utilisateur avant de démarrer : composant partagé `ContentTranslationPanel` exposé par l'admin host (même mécanisme que `MediaPicker`/`RichTextEditor`) + stockage générique `App\Entity\ContentTranslation` indexé par `(entityType, entityId, field, locale)`, et sélecteur public lié au sélecteur de langue d'interface (étape 56). Voir `docs/RELEASE.md` 0.39.0.
