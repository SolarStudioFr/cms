...
| 57 | `i18n-per-plugin-language-files` | Fichiers de langue (FR + EN) pour chacun des plugins restants (Réalisations, Actualités, Accueil, Newsletter, Page builder, Statistiques) | done |
| 58 | `i18n-content-language-picker` | Sélecteur de langue dans les formulaires d'ajout de contenu, **sans dépendance croisée** entre i18n et les plugins de contenu | done |
...
- **Doutes explicitement signalés par l'utilisateur dans `../REFORGE_PLUGINS_AND_I18N.md`** ("Si tu as le moindre doute pose moi des questions") — points à clarifier au démarrage de l'étape concernée plutôt que tranchés unilatéralement ici :
    ...
    - **55** : ~~la page dédiée de gestion des langues (`LangManager.jsx`, étape 07) est-elle remplacée par la nouvelle section "Langues" de Configuration, ou les deux coexistent-elles~~ — tranché avec l'utilisateur au démarrage : remplacée entièrement (une seule UI, voir `docs/RELEASE.md` 0.36.0).
    ...
    - **58** : ~~c'est le point le plus explicitement ouvert du document source...~~ — tranché avec l'utilisateur avant de démarrer : stockage générique `App\Entity\ContentTranslation` indexé par `(entityType, entityId, field, locale)`, et sélecteur public lié au sélecteur de langue d'interface (étape 56). Voir `docs/RELEASE.md` 0.39.0. **UX revue par l'utilisateur en 0.40.0** : le composant partagé `ContentTranslationPanel` (card latérale) a été remplacé par un hook `useContentLocale` dupliqué par plugin + un menu de langue en haut de chaque formulaire qui remplace tous ses champs traduisibles, étendu aux blocs du page builder et à la Newsletter (voir `docs/RELEASE.md` 0.40.0).
